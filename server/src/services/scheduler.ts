...
export async function generateTimetable() {
  ...
  const batches = [];
  if (oddSemesters.length) batches.push(oddSemesters);
  if (evenSemesters.length) batches.push(evenSemesters);
  ...
  for (const batchSemesters of batches) {
    ...
    const occupiedSemesterSlot: Record<string, Set<string>> = {};
    ...
    const isAvailable = (
      ...
      tempBookings: TempBooking[], semester: number
    ) => {
    ...
    const semesterLabsPerDay: Record<number, Record<string, number>> = {};
    ...
    for (const sem of batchSemesters) {
      ...
      for (const subj of semSubjects as any[]) {
        ...
        for (const facultyName of eligibleFaculty) {
          ...
          const subjectDays = new Set<string>(); // Used to prevent more than 1 class (lab or theory) of the same subject per day
          ...
          if (requiredLabs > 0 && labRooms.length > 0) {
            for (let l = 0; l < requiredLabs; l++) {
              ...
              for (const day of shuffle(days)) {
                if (labPlaced) break;
                if (semesterLabsPerDay[sem][day] >= 2) continue; // Max 2 labs per day for the semester
                if (subjectDays.has(day)) continue; // Not more than once per day for the same subject
